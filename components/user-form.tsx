...
export function UserForm({ userId }: UserFormProps = {}) {
    ...
    if (userId) {
      const fetchUser = async () => {
        try {
          ...
          const response = await fetch(`https://67dcde67e00db03c40692a8a.mockapi.io/api/v1/users/${userId}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing
        ? `https://67dcde67e00db03c40692a8a.mockapi.io/api/v1/users/${userId}`
      ...
      toast({
        ...
        description: isEditing ? "The user has been successfully updated." : "The user has been successfully created.",
      })
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {isEditing ? "Edit the student information below" : "Enter the student information below"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Enter a unique identifier for the student (format: B followed by 7 digits)
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ImageUpload value={formData.hinhanh} onChange={handleImageChange} />
            <p className="text-xs text-muted-foreground mt-1">
              Note: In this demo, images are stored temporarily. In a production app, they would be uploaded to a
              server.
            </p>
          </div>
